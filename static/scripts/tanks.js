...
function getState(){
    ...
    distance = Math.round((Math.hypot(bluetank.x-redtank.x,bluetank.y-redtank.y)/10))/gamearea.canvas.width*10;
    angledif = Math.round((bluetank.angle*180/Math.PI - angledif))/360;
    s = [angledif,distance];
    return s;
}
        ...
        if (timeouta >= timeoutamax){                
            ...
            if (redtank.health <=0){
                ...
                if (bluetank.health <= 0){
                    ...
                    distancedifference = Math.round(Math.hypot(bluetank.x-redtank.x,bluetank.y-redtank.y)*10 - distance*gamearea.canvas.width*10)/10/gamearea.canvas.width;
